...
| Theme | Light-pinned Kopi Studio. **Cardinal: cards sit ONE SHADE LIGHTER than the page** — page `bg-background` `#F0E6D6`, card `bg-card` `#FAF6EE`, raised `bg-popover` `#FFFFFF`. Equal bg = invisible cards. No `dark:` variants (dead), no zinc/slate/gray, no saturated Tailwind defaults. Brown CTA + brown focus ring, never flipped. Serif never under 18px. | [light-theme.md](../../.claude/rules/light-theme.md) |
