...
| `<DatePicker>` | Singapore locale · single + range · month-year dropdowns · Today+Clear · year window defaults to SG-year −100…+50 (was a hardcoded 2020–2030; `variant="birth"` on `DateField` narrows it to a DOB range) | 🟢 | 🟢 at `form/DatePicker.tsx` (392 LOC) | 0/many — **JUDGMENT CALL**: sliding range + internal `<select>` overlay for month/year (visual-verify before heavy use) |
