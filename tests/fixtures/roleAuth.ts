 ...
 * Authed specs/projects reuse that session through `storageState` so the in-spec
 * `LoginPage.signIn(...)` becomes an instant no-op (LoginPage detects the
 * /login → /dashboard auto-redirect and returns early).
 *
...
export type AuthRole = keyof typeof testUsers; // 'admin' | 'coordinator' | 'supervisor' | 'storeman'
...
/** storageState path for a role, e.g. authFileFor('coordinator') → tests/.auth/coordinator.json */
