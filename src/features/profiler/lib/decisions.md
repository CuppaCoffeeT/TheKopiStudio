...
## 2026-08-05 — Intake hero rebuilt as the V1 typographic hero (of three user-reviewed options)
**Decision**: The intake masthead becomes a full hero: outcome headline "Know how they think before you pitch." at `clamp(38px, 4vw + 22px, 56px)`, mechanism subheadline, CTA row (primary "Start profiling →" scroll+focuses the Prospect Name input; ghost "See how it works" scrolls to the how-it-works card), and a fact-based trust line ("No account needed — profiles stay on your side of the table", `--fg-dim` on the page ground). The bottom advance button keeps `wizard-start-btn` and relabels "Continue to questions →" so the two CTAs never duplicate. V2 (split + live DISC preview) and V3 (full-height layered) were presented and NOT shipped: both fight the 2a flat-cream/no-illustration direction, and a full-height hero taxes the advisor's 400th daily visit for the sake of the 1st.
**Why**: User-directed hero rebuild (2026-08-05) — strongest section of the page, three options reviewed, typographic version chosen for brand fit and tool ergonomics. Hero CTAs route INTO the existing flow instead of forking it; smooth scrolling checks `prefers-reduced-motion` explicitly since the CSS kill-switch can't override an explicit JS `behavior: 'smooth'`.
**Impact**: `IntakeForm.tsx`; `tests/pom/WizardPage.ts` comment only (specs target the unchanged testid). Question-set freeze untouched — copy here is UI chrome, not persisted content.

## 2026-08-05 — Copy de-duplication pass (design-director critique round)
**Decision**: `QuestionScreen`'s h2 no longer repeats the sticky bar's "Profiling {name}" one inch below it — the heading now carries the instruction ("Weave these into the conversation") and the sub carries the judgement call; `prospectName` prop removed. The idle top-bar kicker "DISC × MBTI · Auto-Profile" (jargon) becomes the outcome line "Read any prospect in one meeting". How-it-works bullets rewritten to advisor language — "Save to database or download" was developer copy on the most client-adjacent screen. Login subline now says something true ("Accounts are provisioned by your administrator — there is no self-serve sign-up") instead of template filler. Age-range placeholder teaches by example ("e.g. 31-35") like every other field.
**Why**: Blunt-review round (2026-08-05): the flow labelled itself three times in 120px, and several strings did no work. All QS/NVG persisted content untouched — these are chrome strings only; no test targets them.
**Flagged, deliberately NOT fixed here**: (1) the five consecutive observation screens (steps 3–7) are the monotony trough of the funnel — collapsing them changes the step count that drafts persist and specs walk, so it needs its own versioned change; (2) the report reveal is where a top agency would spend next (score build-in, printed-report texture) — worth a design handoff, not a drive-by.
**Impact**: `QuestionScreen.tsx`, `ProfilerWizardPage.tsx`, `useWizardController.ts`, `IntakeForm.tsx`, `Login.tsx`.

## 2026-08-05 — Wizard accepts `?prospect=<name>`; disabled-Next explains itself (UX audit)
...
## 2026-08-13 — The profiler links AT SAVE; convert asks before creating a second customer
**Decision**: the CRM entry contract is now BOTH params — `?prospect=<name>` seeds the intake, `?customerId=<uuid>` sets `results.client_id` on the save. The id is re-resolved against the saver's own live book before the insert (`resolveLinkableClientId`), and a requested-but-unlinkable id saves UNLINKED with an explicit toast rather than failing a finished profile. "Convert to client" now asks `findClientByName` first and, on a hit, offers link-to-existing vs create-a-separate-customer — it never inserts blind.
**Why**: `client_id` was only ever written by convert, AFTER the fact, so profiling a customer who already had a record left them reading "Never profiled" on the Overview queue forever (the queue's profiled test is `client_id`, never a name match). Converting then INSERTed a second customer, splitting one person across two rows — observed in prod 2026-08-13, two "Sky Tan" records, the profile on the new one and the contact details on the old.
**Impact**: `savePayload.ts`, `useSaveResult.ts`, `useWizardController.ts` (owns BOTH params now — the page no longer reads the URL), `convertService.ts`, `useConvertResult.ts` (mode auto/link/create), `DuplicateCustomerModal.tsx`, `ResultDetailPage.tsx`; crm side `lib/profilerEntry.ts` is the ONE builder of the entry href. **Supersedes** 2026-08-05 — "?prospect=<name>" (name-only entry).

## 2026-08-19 — The profiler opens as tool 01, and the shell it uses is shared
**Decision**: `/profiler` renders the SAME chrome as tools 04–06 for a signed-in advisor — `ToolPageHeader` ("01 Prospect Profiler", breadcrumb, description from `TOOL_ROUTES`) over `ToolCustomerBar`, on `ToolPageShell` — while an anonymous visitor keeps `IntakeHero` + `WizardTopBar` untouched. The chrome was HOISTED out of `features/crm/planning/components/` into `@/components/primitives/tools`; `PlanningAtoms.tsx` and `PlanningToolHeader.tsx` are deleted. `Card` + `Eyebrow` become `ToolPanel` across the wizard and the report.
**Why**: `src/lib/toolRoutes` has always listed the profiler as tool 01 beside the tax calculator, the SRS planner and the Legacy Map, and the nav rail renders them as one band — but the page rendered none of their language, so one rail led to two apps. The panel treatment is also the one KOPI_2A_SPEC actually specifies ("Archetype — detail" → Panels: uppercase 11px `.12em` muted label); `Eyebrow` is the MASTHEAD kicker (600 11px `.14em`) and was doing a panel label's job. `Eyebrow` survives where it belongs — in `IntakeHero`, on the page cream, where `ToolPanel`'s muted label would not hold 4.5:1.
**Why hoist rather than import**: `.dependency-cruiser` `no-cross-feature-imports` — `profiler` may not read `crm`. Only the MARKUP moved; the DATA stayed. `ToolCustomerBar` is presentational and each feature wraps it with its own hook, so hoisting it did not put the customer record in a shared lane (the move planning/decisions.md 2026-07-28 rejected).
**Two deliberate deviations from tools 04–06**: (1) `ToolPageShell measure="reading"` (`max-w-[42rem]`), not `wide` (`max-w-5xl`) — the wide measure exists to hold their `1fr 360px` input/summary grid, and their real reading column is ~640px; a single column of prose and four-option radio rows would stretch to 1024px. Field widths land within ~50px of the tax tool's either way. (2) The shell renders on the INTAKE screen only — in flow the sticky progress rail is the page's identity, and a 38px masthead over all seven steps is chrome, not orientation.
**Impact**: new `src/components/primitives/tools/` (+ its CONTEXT.md); `PlanningToolFrame` keeps only the state; `crm/components/ToolCustomerBar` is now a data wrapper. Profiler: `ProfilerWizardPage`, `IntakeForm`, `QuestionScreen`, `ObservationScreen`, and every report section. All `data-testid`s preserved — verified against `WizardPage` POM.

## 2026-08-19 — The picker writes the entry contract, it does not invent a second one
**Decision**: the intake screen's customer picker writes `?customerId=<uuid>&prospect=<name>` — byte-for-byte the pair `crm/lib/profilerEntry.profilerHrefFor` writes — and fills `intake.name` directly. It does NOT adopt `CUSTOMER_PARAM` (`?customer=`), which tools 04–06 use.
**Why**: `client_id` is the load-bearing half of "profiled" (2026-08-13 above), and it already had exactly one URL spelling. Giving the picker a second spelling would have meant two ways to say the same thing on one route and a migration of every existing link, bookmark and spec for a param name nobody sees. Arriving from the CRM and picking in the bar now converge on one URL instead of forking.
**Behaviour**: picking OVERWRITES a typed name (you chose a record, the record wins); Clear drops the link and LEAVES the text (unlinking is not erasing). `replace: true` — choosing a customer is not a place you navigate back to, and Back already means "previous step".
**Impact**: `useWizardController.chooseCustomer`, new `api/customerOptionsService.ts` + `hooks/useOwnCustomerOptions.ts`. The hook shares `queryKeys.crmClients.ownOptions` with the CRM picker on purpose — one cache entry, so adding a customer there refreshes this list too, without either feature importing the other.

## 2026-08-19 — "All 4 answered" was a lie from the first render
**Decision**: `answeredInBatch` tests `wizard.answers[qi] !== null`, not `!== undefined`.
**Why**: `useWizardState` seeds `answers` with `new Array(QS.length).fill(null)`, so every slot is DEFINED before a single option is picked. The aria-live hint added on 2026-08-05 specifically to explain a disabled Next has therefore always read "All 4 answered" beside a disabled button — the exact confusion it was built to remove, announced to screen readers. `isBatchComplete` has always used `!== null`, which is why the gating was right and only the explanation was wrong; that mismatch is what hid it.
**Impact**: `ProfilerWizardPage.tsx`. Found by driving the flow in a browser during the tool-shell work, not by a test — no spec asserts the hint's text.

## 2026-08-19 — Where the tool-shell work landed after the LOC ratchet
**Decision**: three files crossed the 200-LOC ceiling and were decomposed at real seams, not trimmed. `ToolAtoms.tsx` → `ToolPanel` / `ToolFigures` (ToolStatGrid + SummaryRow) / `ToolSelect` (+ ToolNote). `ProfilerWizardPage` → `WizardToolHeader` (the signed-in front door: header + picker + its own-book query) and `WizardStickyHeader` (the top bar and progress rail, which must stay ONE nested block — pinning them separately needed a hardcoded `top-[53px]` that broke whenever the bar's height changed). `useWizardController` → `useCustomerLink`.
**Why the seams are real**: `WizardToolHeader` holds everything about "which customer is this for", a question the wizard below it never asks again — and mounting it conditionally is what parks the picker's query, so the split removed a prop that existed only to do that. `useCustomerLink` is the better outcome of the three: the entry contract's READ (arriving from the CRM) and WRITE (picking in the bar) now sit in one file, so the two doorways cannot spell the same params differently — which was the whole point of the 2026-08-13 entry above.
**Why the atoms split cost nothing**: every consumer imports the barrel `@/components/primitives/tools`, never a deep path, so three new files touched zero call sites. Keep it that way.
**Impact**: ratchet back to baseline (34 files over 200). Re-verified after the split: 675 unit tests, dependency-cruiser clean, and the wizard-anonymous · load-a11y · crm/tool-routes suites all pass.
