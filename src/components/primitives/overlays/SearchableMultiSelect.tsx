    ...
    <div className={className}>
      ...
      <PopoverPrimitive.Root open={open} onOpenChange={disabled ? undefined : setOpen}>
          ...
          <button
            ...
            className={cn(
              ...
              disabled
                ...
                : isBare
                  ? open
                    ? 'bg-card ring-2 ring-ring cursor-pointer'
          ...
          >
            ...
            {selectedOptions.length > 0 && isMulti && !hideSelectedPills && (
              <>
                ...
                {overflow > 0 && (
                  <span
                    className="text-[12px] px-2 py-[3px] rounded-full bg-secondary text-[color:var(--fg-dim)]"
        ...
        <PopoverPrimitive.Content
          ...
          >
            {searchable && (
              <div className="flex-shrink-0 border-b border-border px-3 py-2.5 flex items-center gap-2 focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring/15">
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        // --row-selected (brown 12%), not the flat tint: the tint
                        // is ~1.1:1 on the cream popover and the active row is the
                        // only keyboard-position indicator this list has.
                        active && !o.disabled ? 'bg-[color:var(--row-selected)]' : '',
              ...
              <div className="flex-shrink-0 border-t border-border p-1">
                <button
                  ...
                  className={cn(
                    ...
                    'text-[13px] font-medium text-[color:var(--brown-text)]',
                    'bg-[color:var(--accent-red-soft-bg)] hover:bg-[color:var(--row-selected)]'
                  )}
                >
            ...
            <div className="flex-shrink-0 border-t border-border bg-secondary px-3 py-2 flex gap-3 items-center">
              <span className="text-[10px] text-[color:var(--fg-dim)] flex items-center gap-1" style={{ fontFamily: 'var(--font-mono)' }}>
                <Kbd>↑↓</Kbd> navigate
              </span>
              <span className="text-[10px] text-[color:var(--fg-dim)] flex items-center gap-1" style={{ fontFamily: 'var(--font-mono)' }}>
    ...
    return (
      <span
        className={cn(
          ...
          selected
            ? 'bg-primary border-primary text-[color:var(--cta-primary-fg)]'
            : 'border-border bg-transparent'
        )}
      >
        {selected && (
          <svg width="10" height="10" viewBox="0 0 10 10">
            <path d="M2 5 L4.5 7.5 L8 3" stroke="currentColor" strokeWidth="1.6" fill="none" strokeLinecap="round" />
          </svg>
        )}
      </span>
    );
  }
  // --brown-text, not raw --primary: the glyph carries selection state at 14px
  // and raw brown only reaches 4.24:1 on the highlighted row fill.
  return (
    <span className="w-3.5 h-3.5 mt-[3px] flex-shrink-0 inline-flex items-center justify-center text-[color:var(--brown-text)]">
      {selected && (
...
function highlight(text: string, q: string): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="bg-[color:var(--row-selected)] text-[color:var(--brown-text)] rounded px-[1px]">
