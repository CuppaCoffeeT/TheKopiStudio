...
A new module needs **no nav wiring**: all three navigation surfaces read `useAuth().modules` — `AppSidebar` (the >= lg rail) and `AppNavDrawer` (the < lg drawer, opened from the mobile bar's menu button) share one `AppSidebarNav` component, and the `CommandPalette` (opened from the mobile bar's search icon — the ⌘K hotkey was removed 2026-08-05) reads the same list.
